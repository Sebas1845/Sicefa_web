...
@section('content')
                            ...
                            <tbody>
                                @foreach ($permissions as $permission)
                                    <tr>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($permission->date)->format('d/m/Y H:i') }}</td>
                                        ...
                                        <td class="text-center">
                                            @if ($permission->permissionValidations->isNotEmpty())
                                                <button type="button" class="btn btn-sm btn-outline-primary"
                                                    ...
                                                    data-bs-target="#modalEstado{{ $permission->id }}">
                                        ...
                                        @if ($permission->permissionValidations->isNotEmpty())
                                            <div class="modal fade" id="modalEstado{{ $permission->id }}" tabindex="-1"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body modal-body-adapt">
                                                            @foreach ($permission->permissionValidations as $validation)
                                                                <dl class="row mb-0">
                                                                    <dt class="col-sm-5 fw-semibold">Validado por:</dt>
                                                                    <dd class="col-sm-7">
                                                                        {{ $validation->validator->full_name ?? 'No disponible' }}
                                                                        <small class="text-muted fst-italic">
                                                                            ({{ $validation->validator_role ?? 'Rol no asignado' }})
                                                                        </small>
                                                                    </dd>

                                                                    <dt class="col-sm-5 fw-semibold">Resultado:</dt>
                                                                    <dd class="col-sm-7">
                                                                        ...
                                                                        @else
                                                                            <span
                                                                                class="badge bg-secondary">{{ ucfirst($validation->validation_status) }}</span>
                                                                        @endif
                                                                    </dd>

                                                                    <dt class="col-sm-5 fw-semibold">Observaciones:</dt>
                                                                    <dd class="col-sm-7">
                                                                        {{ $validation->observation ?? 'Sin observaciones' }}
                                                                    </dd>

                                                                    <dt class="col-sm-5 fw-semibold">Fecha de validación:
                                                                    </dt>
                                                                    <dd class="col-sm-7">
                                                                        {{ \Carbon\Carbon::parse($validation->validated_at ?? $validation->created_at)->format('d/m/Y H:i') }}
                                                                    </dd>
                                                                </dl>
                                                                @if (!$loop->last)
                                                                    <hr>
                                    ...
                                    <div class="modal fade" id="validateModal{{ $permission->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <form method="POST"
                                                    action="{{ route('sigac.bienestar.PermissionValidation.store') }}"
                                                    ...
                                                    <div class="modal-body modal-body-adapt">
                                                        ...
                                                        <div class="mb-3"
                                                            ...
                                                            <textarea name="observation" id="observation_{{ $permission->id }}" class="form-control" rows="4" required></textarea>
                                                        </div>
    ...
    </div><div class="modal fade" id="evidenceModal" tabindex="-1" aria-labelledby="evidenceModalLabel" aria-hidden="true">
...
{{-- Script funcional adaptado --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {

        // Detectar disponibilidad de SweetAlert
        ...
        // Captura qué botón disparó el modal (usa evento de Bootstrap)
        evidenceModal.addEventListener('show.bs.modal', function(event) {
            ...
            if (!evidenceUrl) {
                evidenceContainer.innerHTML = '<p class="text-muted">No se encontró evidencia.</p>';
                return;
            }

            activeBtn = btn;
            evidenceContainer.innerHTML = '<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Cargando...</span></div>';
            ...
            // Cargar evidencia después de un breve delay
            setTimeout(() => {
                if (evidenceType === 'pdf') {
                    evidenceContainer.innerHTML = `
                        <iframe src="${evidenceUrl}" width="100%" height="600px" frameborder="0"></iframe>
                    `;
                } else if (evidenceType === 'image') {
                    evidenceContainer.innerHTML = `
                        <img src="${evidenceUrl}" alt="Evidencia" class="img-fluid rounded shadow">
                    `;
                } else {
                    evidenceContainer.innerHTML = `
                        <p class="text-muted">Tipo de evidencia no soportado o archivo no encontrado.</p>
                    `;
                }
            }, 300);
        });

        // Restablece el contenido al cerrar el modal
        ...
        @foreach ($permissions as $permission)
            (function() {
                ...
                const obsContainer = document.getElementById('observation_container_{{ $permission->id }}');
                ...
                // Mostrar/ocultar observación si rechaza
                if (selectEl) {
