...
<td>
    @if ($buttonDisabled)
        <button type="button" class="btn btn-sm btn-secondary" disabled title="Ya fue validado por el Coordinador Académico">
            <i class="bi bi-lock"></i> Bloqueado
        </button>
    @else
        <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#validateModal{{ $validation->id }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    evidenceModal.addEventListener('show.bs.modal', function(event) {
        ...
        const evidenceUrl = button?.getAttribute('data-evidence');
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext)) {
            html = `<img src="${evidenceUrl}" class="img-fluid rounded shadow mt-2" alt="Evidencia">`;
        } else if (ext === 'pdf') {
            html = `<iframe src="${evidenceUrl}" width="100%" height="600px" frameborder="0"></iframe>`;
        } else {
            html = `<p class="text-muted mt-3">Tipo de archivo no soportado.</p>`;
        }

        // Reemplazar spinner por la evidencia tras un pequeño delay
        setTimeout(() => {
