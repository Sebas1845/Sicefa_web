...
@section('content')
    ...
    <!-- ✅ Modal global para visualizar evidencias -->
   ...
   {{-- ✅ Script adaptado completamente --}}
<script>
    const swalAvailable = (typeof Swal !== 'undefined');

    document.addEventListener('DOMContentLoaded', function() {

        // 🔹 Ver Evidencia (botón con el ícono de ojo)
        ...
        if (evidenceButtons && evidenceModal && evidenceContainer) {
            evidenceButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    setTimeout(() => {
                        if (evidenceUrl.match(/\.(jpeg|jpg|png|gif|webp)$/i)) {
                            // 🖼️ Imagen
                            evidenceContainer.innerHTML = `
                            ...
                            `;
                        } else if (evidenceUrl.match(/\.pdf$/i)) {
                            // 📄 PDF
                            evidenceContainer.innerHTML = `
                    ...
                    }, 300); // pequeña espera para suavizar carga
                });
            });

            // Limpia contenido cuando se cierra el modal
            evidenceModal.addEventListener('hidden.bs.modal', () => {
                evidenceContainer.innerHTML = '<p class="text-muted">Cargando evidencia...</p>';
            });
        }

        // 🔹 Lógica de validación (manteniendo tu estilo actual)
        @foreach ($permissions as $permission)
            (function() {
                ...
                if (selectEl && obsContainer && obsField) {
                    ...
                    toggleObservation();
                    selectEl.addEventListener('change', toggleObservation);
                }
                ...
                if (formValidate) {
                    formValidate.addEventListener('submit', function(e) {
                        ...
                        const confirmAction = () => formValidate.submit();
                        ...
                        if (swalAvailable) {
                            ...
                            if (confirm('¿Estás seguro?')) confirmAction();
                        }
                    });
                }
            })();
