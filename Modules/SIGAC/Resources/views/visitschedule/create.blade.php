...
(function() {
  ...
  window.validateReady = function() {
    ...
    const timeOk      = hasStart && hasEnd && (startTime.value < endTime.value);
    const hasPerson   = !!personId.value;

    const baseOk = hasActivity && hasDate && timeOk && hasPerson;

    const envOk  = noEnvCB.checked || (envSelect.value !== '');
  ...
  async function fetchEnvironments() {
    if (noEnvCB.checked) { window.validateReady(); return; }
  ...
  // Recalcular cuando el usuario escriba o cambie valores
  [activity, date, startTime, endTime].forEach(el => {
    el.addEventListener('change', () => {
      fetchEnvironments();
      window.validateReady();
    });
    el.addEventListener('input', window.validateReady);
  });

  envSelect.addEventListener('change', window.validateReady);
