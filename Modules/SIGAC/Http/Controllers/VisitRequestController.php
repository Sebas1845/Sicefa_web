    ...
    public function application_index()
{
    $visitRequests = VisitRequest::with([
        ...
        'schedules.personInCharge', // <- AQUI (cargar encargado)
    ])->orderByDesc('id')->get();
    ...
    public function application_store(Request $request)
    {
        $validated = $request->validate([
            ...
            'practice_requirements' => 'nullable|string|max:2000|required_if:type,practica',
        ], [
            ...
            'practice_requirements.required_if' => 'Describe los requerimientos para la práctica.',
        ]);
        ...
        if ($request->hasFile('people_list')) {
            ...
            $dir  = 'sigac/visit_people_lists'; // quedará en public/modules/sigac/files/visit/visit_people_lists
    ...
    {
        return view('sigac::visits.report_form', [
            'titlePage' => 'Reporte de visitas',
            'titleView' => 'Reporte de visitas',
        ]);
    }
    ...
    public function export(Request $request)
    {
        ...
        // Nota: filtramos por fecha de agenda (visit_schedules.date)
        $visits = VisitRequest::with(['company', 'person', 'schedules.environment', 'schedules.personInCharge'])
        ...
        if ($request->format === 'excel') {
            // Opción A (rápida): exportar con una Export class (recomendado)
            // return Excel::download(new VisitsReportExport($visits, $start, $end), 'reporte_visitas.xlsx');

            // Opción B (sin Export class): devuelve vista para depurar o arma CSV.
            return response()->json([
                'count' => $visits->count(),
                'message' => 'Configura el Export (VisitsReportExport) para descargar Excel.',
            ]);
        }

        // PDF (ej. barryvdh/laravel-dompdf)
        // $pdf = \PDF::loadView('sigac::visits.report_pdf', compact('visits','start','end'));
        // return $pdf->download('reporte_visitas.pdf');

        return response()->json([
            'count' => $visits->count(),
            'message' => 'Configura la vista + librería PDF para descargar.',
        ]);
    }
}
