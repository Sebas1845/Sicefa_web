{{-- Modules/GDF/Resources/views/public/home.blade.php --}}
...
@php
    $hasAuth = auth()->check();
    $hasCheckRol = $hasAuth && function_exists('checkRol');
    ...
    // Acciones principales (solo si hay sesión y roles)
    $primaryAction = null;
    ...
    // Links públicos (ajusta a tus rutas reales)
    $linkDevs  = \Illuminate\Support\Facades\Route::has('gdf.public.developers') ? route('gdf.public.developers') : '#';
    $linkAbout = \Illuminate\Support\Facades\Route::has('gdf.public.about') ? route('gdf.public.about') : '#';
    $linkTech  = \Illuminate\Support\Facades\Route::has('gdf.public.tech') ? route('gdf.public.tech') : '#';
@endphp

{{-- Barra pública simple (si ya tienes navbar en master, puedes borrar esto) --}}
<div class="d-flex justify-content-end flex-wrap gap-2 mb-3">
    <a class="btn btn-outline-secondary btn-sm" href="{{ $linkAbout }}">
        <i class="bi bi-info-circle"></i> Sobre el software
    </a>
    <a class="btn btn-outline-secondary btn-sm" href="{{ $linkDevs }}">
        <i class="bi bi-people"></i> Desarrolladores
    </a>
    <a class="btn btn-outline-secondary btn-sm" href="{{ $linkTech }}">
