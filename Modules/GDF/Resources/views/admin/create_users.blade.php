...
@section('title','GDF | Usuarios y roles')

@section('content')
@php
    $isAdminGdf = function_exists('checkRol') ? checkRol('gdf.admin') : false;
    if(!$isAdminGdf){ abort(403); }

    $pretty = function (string $slug): string {
        return match($slug) {
            'gdf.admin'                 => 'Admin GDF',
            'gdf.subdirection'          => 'Subdirección',
    ...
    $src      = $src ?? request('src'); // contractor|employee|null
@endphp
    ...
    <div class="col-12">
        <div class="gdf-shell p-4">
            ...
            <div class="d-flex align-items-start justify-content-between flex-wrap gap-3">
                <div>
                    ...
                    <p class="gdf-muted mb-0">
                        Administra roles del módulo GDF (<span class="fw-semibold">gdf.*</span>).
                    </p>
                </div>

                <div class="d-flex gap-2 flex-wrap">
                    <a href="{{ route('gdf.admin.dashboard') }}" class="btn btn-gdf-ghost btn-sm">
                        <i class="bi bi-arrow-left"></i> Volver
                    </a>

                    <button type="button" class="btn btn-success btn-sm"
                            data-bs-toggle="modal" data-bs-target="#createUserByDocModal">
                        <i class="bi bi-person-plus"></i> Crear usuario (Cédula)
                    </button>

                    {{-- Importar = personas SIN usuario --}}
                    <a class="btn btn-outline-light btn-sm"
                       href="{{ route('gdf.admin.users.index', array_merge(request()->query(), ['import_type'=>'both'])) }}">
            ...
            @foreach (['success'=>'success','warning'=>'warning','info'=>'info','error'=>'danger'] as $k=>$type)
                @if(session($k))
                    <div class="alert alert-{{ $type }}">{{ session($k) }}</div>
                @endif
            @endforeach

            {{-- Filtros --}}
            <form class="row g-2 mb-3" method="GET" action="{{ route('gdf.admin.users.index') }}">
                <div class="col-md-4">
                ...
                <div class="col-md-3">
                    <select name="role_slug" class="form-select">
                        <option value="">— Todos los roles GDF —</option>
                ...
                <div class="col-md-5 d-flex gap-2 flex-wrap">
                    ...
                    {{-- Ver solo usuarios del sistema --}}
                    <a class="btn {{ $onlyUsers ? 'btn-light' : 'btn-outline-light' }}"
                       href="{{ route('gdf.admin.users.index', array_merge(request()->query(), ['only_users' => 1])) }}">
                        <i class="bi bi-people"></i> Usuarios
                    </a>

                    {{-- Filtro por origen --}}
                    <a class="btn {{ $src==='contractor' ? 'btn-light' : 'btn-outline-light' }}"
                       href="{{ route('gdf.admin.users.index', array_merge(request()->query(), ['src' => 'contractor'])) }}">
                        <i class="bi bi-person-badge"></i> Contractors
                    </a>

                    <a class="btn {{ $src==='employee' ? 'btn-light' : 'btn-outline-light' }}"
                       href="{{ route('gdf.admin.users.index', array_merge(request()->query(), ['src' => 'employee'])) }}">
                ...
                @forelse($users as $u)
                    ...
                    <div class="gdf-card2">
                        ...
                        <div class="gdf-box">
                            ...
                            @else
                                <form method="POST" action="{{ route('gdf.admin.users.assignRole') }}" class="d-flex gap-2">
                                    @csrf
                        ...
                        <div class="gdf-box mt-2">
                            ...
                            @else
                                <form method="POST" action="{{ route('gdf.admin.users.revokeRole') }}" class="d-flex gap-2">
                                    @csrf
                                    <input type="hidden" name="user_id" value="{{ $u->id }}">
                                    <select name="role_slug" class="form-select form-select-sm" required>
                        ...
                        <div class="gdf-box mt-2">
                            ...
                            @else
                                {{-- ✅ Ajusta esta ruta al nombre real de TU método de envío --}}
                                <form method="POST" action="{{ route('gdf.admin.users.sendResetLink', $u->id) }}">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-warning btn-sm w-100"
                                            onclick="return confirm('¿Enviar enlace a {{ $email }}?');">
                                        <i class="bi bi-envelope"></i> Enviar acceso por correo
                                    </button>
                                </form>
                                <div class="gdf-muted small mt-2">
                                    Envía un enlace para que el usuario cree su contraseña.
                                </div>
                            @endif
                        </div>

                        <div class="gdf-muted small mt-2">
                            Solo roles <span class="fw-semibold">gdf.*</span>.
                        </div>
                    </div>
                @empty
...
{{-- MODAL: Crear usuario por cédula --}}
<div class="modal fade" id="createUserByDocModal" tabindex="-1" aria-labelledby="createUserByDocModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form method="POST" action="{{ route('gdf.admin.users.createFromContractor') }}">
      @csrf
      <div class="modal-content gdf-modal">
        <div class="modal-header">
          <h5 class="modal-title" id="createUserByDocModalLabel"><i class="bi bi-person-plus"></i> Crear usuario por cédula</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>

        <div class="modal-body">
          <p class="gdf-muted mb-2">
            Busca la cédula en <span class="fw-semibold">contractors</span>. Si existe, se crea el usuario.
          </p>
          ...
          <div class="gdf-muted small">Luego asigna roles desde la tarjeta.</div>
        </div>

        <div class="modal-footer">
          <button type="button" class="btn btn-outline-light" data-bs-dismiss="modal">Cancelar</button>
          <button type="submit" class="btn btn-success"><i class="bi bi-check2-circle"></i> Crear usuario</button>
        </div>
      </div>
    </form>
  </div>
...
<div class="modal fade" id="importUsersModal" tabindex="-1" aria-labelledby="importUsersModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-scrollable">
    <div class="modal-content gdf-modal">
      <div class="modal-header">
        <h5 class="modal-title" id="importUsersModalLabel"><i class="bi bi-people-fill"></i> Importar usuarios (sin usuario)</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>

      <div class="modal-body">
        <div class="alert alert-info small">
          Selecciona personas desde <b>Contractors</b> o <b>Employees</b> que aún no tienen usuario.
        </div>

        {{-- Controles para cargar --}}
        <form method="GET" action="{{ route('gdf.admin.users.index') }}" class="row g-2 mb-3">
        ...
        {{-- Form crear --}}
        <form method="POST" action="{{ route('gdf.admin.users.import.create') }}">
            @csrf
            ...
            <div class="modal-footer mt-3">
                ...
                <button type="submit" class="btn btn-success"
                        onclick="return confirm('¿Crear usuarios seleccionados y enviar enlace de acceso?');">
                    <i class="bi bi-send-check"></i> Crear y enviar acceso
                </button>
            </div>
        </form>
...
{{-- Autoabrir modal si import_type viene en la URL --}}
