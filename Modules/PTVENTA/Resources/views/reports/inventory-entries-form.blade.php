...
@push('breadcrumbs')
    ...
    <li class="breadcrumb-item active">{{ trans('ptventa::reports.Breadcrumb_Active_Inventory_Entries_1') }}</li>
@endpush
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12">
                    <div class="row mb-3">
                        <div class="col-md-auto">
                            <form class="form-inline"
                                action="{{ route('ptventa.' . getRoleRouteName(Route::currentRouteName()) . '.reports.inventory.entries.generate') }}"
                                method="POST">
                                @csrf
                                ...
                                @if (Auth::user()->havePermission(
                                        'ptventa.' . getRoleRouteName(Route::currentRouteName()) . '.reports.inventory.entries.generate'))
                        ...
                        <div class="col-md">
                            @if (Auth::user()->havePermission(
                                    'ptventa.' . getRoleRouteName(Route::currentRouteName()) . '.reports.inventory.entries.pdf'))
                                <form
                                    action="{{ route('ptventa.' . getRoleRouteName(Route::currentRouteName()) . '.reports.inventory.entries.pdf') }}"
                                    method="post">
                                    @csrf
                                    <input type="hidden" name="start_date" value="{{ $start_date }}">
                                    <input type="hidden" name="end_date" value="{{ $end_date }}">
                                    <button type="submit"
                                        class="btn btn-danger">{{ trans('ptventa::reports.Btn_Generate_PDF') }} <i
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                <thead class="table-dark">
                                    <tr>
                                        <th class=text-center>{{ trans('ptventa::reports.1T_Number') }}</th>
                                        <th class="text-center">{{ trans('ptventa::reports.1T_Voucher') }}</th>
                                        <th>{{ trans('ptventa::reports.1T_Responsible_Delivery') }}</th>
                                        <th>{{ trans('ptventa::reports.1T_Registration_Date') }}</th>
                                        <th>{{ trans('ptventa::reports.1T_Product') }}</th>
                                        <th class="text-center">{{ trans('ptventa::reports.1T_Amount') }}</th>
                                        <th class="text-center">{{ trans('ptventa::reports.1T_Price') }}</th>
                                        <th class="text-center">{{ trans('ptventa::reports.1T_Subtotal') }}</th>
                                        <th class="text-center">{{ trans('ptventa::reports.1T_Total') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                    ...
                    @else
                        <p>{{ trans('ptventa::reports.1T_Text_Optional') }}</p>
                    @endif
                </div>
            </div>
        </div>
