...
class Person extends Model implements Auditable
{
    ...
    { // Obtener la edad actual a partir de la fecha nacimiento (ACCESOR)
    ...
    { // Obtener el nombre completo de la persona first_name + first_last_name + second_last_name (ACCESOR)
        return $this->first_name . ' ' . $this->first_last_name . ' ' . $this->second_last_name;
    }
    public function getIdentificationTypeNumberAttribute()
    { // Obtener de manera abreviada del tipo y número de identificación
    ...
    { // Convierte el primer carácter en mayúscula del dato address (MUTADOR)
        $this->attributes['address'] = ucfirst($value);
    }
    public function setFirstLastNameAttribute($value)
    { // Convertir a mayúsculas en valor del dato first_last_name (MUTADOR)
        return $this->attributes['first_last_name'] = mb_strtoupper($value);
    }
    public function setFirstNameAttribute($value)
    { // Convertir a mayúsculas en valor del dato first_name (MUTADOR)
        return $this->attributes['first_name'] = mb_strtoupper($value);
    }
    public function setSecondLastNameAttribute($value)
    { // Convertir a mayúsculas en valor del dato second_last_name (MUTADOR)
        return $this->attributes['second_last_name'] = mb_strtoupper($value);
    }

    public function setFirstLastNameAttributeNIT($value)
    { // Establecer first_last_name como null cuando el tipo de documento sea "Número de Identificación Tributaria"
    ...
    // RELACIONES
    public function academic_programmings()
    { // Accede a todos los registros de programaciones academicas asociadas a este persona designada como instructor
        return $this->hasMany(AcademicProgramming::class, 'instructor_id');
    }
    public function apprentices()
    { // Accede a todos aprendices que han sido asociados con esta persona
    ...
    { // Accede a todas los registros de las personas que han sido autorizados para votar
        return $this->hasMany(Authorized::class);
    }
    public function cash_counts()
    { // Accede a todas las sesiones de caja asociados a esta persona
        return $this->hasMany(CashCount::class);
    }
    public function courses()
    { // Accede a todos los cursos que han sido asociados con esta persona
        return $this->hasMany(Course::class);
    }
    public function evaluative_judgments()
    { // Accede a todos los juicios evaluativos de esta persona
        return $this->hasMany(EvaluativeJudgment::class);
    }
    public function learning_outcomes()
    { //Accede a todas los resultados de aprendizaje que pertenecen a este perfil (PIVOTE)
        return $this->belongsToMany(LearningOutcome::class, 'learning_outcome_people');
    }
    public function learning_outcome_people()
    { // Accede a todos los registros de contratistas que le pertenecen a esta persona
    ...
    public function supervisor_contractors()
    { // Accede a todos los registros de supervisores de contratación que le pertenecen a esta persona
        return $this->hasMany(Contractor::class, 'supervisor_id');
    }
    public function employees()
    { // Accede a todos los registros de empleados que pertenecen a esta persona
        return $this->hasMany(Employee::class);
    }
    public function e_p_s()
    { // Accede a la EPS que se encuentra asociado(a)
    ...
    { // Accede a todos los eventos que ha asistido esta persona (PIVOTE)
    ...
    { // Accede a todas las granjas que lidera esta persona
        return $this->hasMany(Farm::class);
    }
    public function familypersonfootprints()
    { // Accede a todas las familias que consumen este recuerso
        return $this->hasMany(FamilyPersonFootprint::class);
    }
    public function formulations()
    { // Accede a todos los registros de formulaciones que tiene esta persona.
        return $this->hasMany(Formulation::class);
    }
    public function inventories()
    { // Accede a todos los registros de inventarios que estan a cargo de esta persona
    ...
    public function instructor_programs()
    { // Accede a todas las programaciones de este instructor
        return $this->hasMany(InstructorProgram::class);
    }
    public function juries()
    { // Accede a todos los jurados que están registrados con esta persona
        return $this->hasMany(Jury::class);
    }
    public function labors()
    { // Accede a todas las labores que han sido asignados a esta persona
        return $this->hasMany(Labor::class);
    }
    public function movement_responsibilities()
    { // Accede a todos los registros de responsables de movimiento que ha sido participe esta persona
        return $this->hasMany(MovementResponsibility::class);
    }
    public function municipality_events()
    { // Accede a todos los registros de eventos en municipios que le pertenecen a esta persona
        return $this->hasMany(MunicipalityEvent::class);
    }
    public function pension_entity()
    { // Accede a la entidad de pensiones al que pertenece
        return $this->belongsTo(PensionEntity::class);
    }
    public function population_group()
    { // Accede al grupo poblacional que pertenece
        return $this->belongsTo(PopulationGroup::class);
    }
    public function pqrs()
    { // Accede a todos los pqrs que pertenecen a esta persona (PIVOTE)
        return $this->belongsToMany(Pqrs::class)->withPivot('date_time', 'type')->withTimestamps();
    }
    public function productive_units()
    { // Accede a todas las unidades productivas que lidera esta persona
        return $this->hasMany(ProductiveUnit::class);
    }
    public function professions()
    { //Accede a todas las profesiones que tiene esta persona.
        return $this->belongsToMany(Profession::class, 'person_professions');
    }
    public function request_externals()
    { // Accede a todas las solicitudes externas que le pertenecen esta persona
    ...
    { // Accede a todos los usuarios registrados con esta persona
    ...
    public function areaBudgetAssignments()
    {
    ...
    {
        return $this->areaBudgetAssignments()
            ->active()
            ->currentlyValid();
    }
}
